...
__attribute__((unused)) static void Init(std::string pcie_addr, int logging_queue_num,
  ...
  // lemma topfs_cache = new HHVMLRUCache<uint64_t, std::shared_ptr<LRUEntry>>(SPDK_MEM_POOL_ENTRY_NUM - 2000);
...
class SpdkWritableFile : public FSWritableFile {
 public:
  ...
  IOStatus Close(const IOOptions& /**/, IODebugContext* /**/) override { return file_->Close(); }
